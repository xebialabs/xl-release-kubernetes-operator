...
### B.2. Prepare the release operator 
...
Compare values in the CR path `spec.ImageTag` and match them to the `default` namespace value in that case.  
:::

### B.3. Update the release operator package to support custom namespace (common part)
...
### B.5. Prepare to deploy to the custom namespace
...
```
❯ kubectl get pvc dai-xlr-digitalai-release -n default -o yaml > dai-xlr-digitalai-release.yaml
```
...
### B.8. Move existing PVC to the custom namespace

Delete the current PVC in the namespace `default` if it still exists:
```
...
Edit file `dai-xlr-digitalai-release.yaml`:
...
- `metadata.finalizers`
- `metadata.annotations` (do not delete `metadata.annotations.meta.helm.sh/release-name` and `metadata.annotations.meta.helm.sh/release-namespace`)
...
Create those PVCs again, but inside the Namespace “integration”:
```
❯ kubectl apply -f dai-xlr-digitalai-release.yaml -n custom-namespace-1
...
### B.8. Deploy to the custom namespace

1. Do the step 7 from the documentation [Step 7—Activate the deployment process](https://docs.xebialabs.com/v.10.3/deploy/how-to/k8s-operator/install-deploy-using-k8s-operator/#step-7activate-the-deployment-process-1)
2. Do the step 8 from the documentation [Step 8—Verify the deployment status](https://docs.xebialabs.com/v.10.3/deploy/how-to/k8s-operator/install-deploy-using-k8s-operator/#step-8verify-the-deployment-status-1)


### B.9. Apply any custom changes
...
#### B.7.a Destroy remains of XLR in default namespace
