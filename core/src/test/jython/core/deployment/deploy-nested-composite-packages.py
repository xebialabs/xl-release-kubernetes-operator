...
compApp2_0 = repository.create(factory.configurationItem('%s/2.0' % compApp.id, 'udm.CompositePackage', {"packages":["%s/1.0" % compApp.id, "Applications/DeploymentApp4/1.0"]}))

depl = deployment.prepareInitial(compApp2_0.id, env.id)
...
assertEquals("Deploy %s 2.0 on %s" % (CiUtils.getName(compApp.id), CiUtils.getName(env.id)), rblock.description)
