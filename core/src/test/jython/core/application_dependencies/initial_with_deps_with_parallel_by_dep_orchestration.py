...
depl.deployedApplication.values['orchestrator'] = ['parallel-by-dependency']
...
assertEquals(1, depl.requiredDeployments[0].deploymentGroupIndex)
assertEquals(1, len(depl.requiredDeployments[1].deployeds))
assertEquals(2, depl.requiredDeployments[1].deploymentGroupIndex)
assertEquals(1, len(depl.requiredDeployments[2].deployeds))
assertEquals(2, depl.requiredDeployments[2].deploymentGroupIndex)
deployeds = sorted(map(lambda x : x.deployeds[0].name, depl.requiredDeployments))
assertEquals("backendDeployable", deployeds[0])
assertEquals("databaseDeployable", deployeds[1])
assertEquals("servicesDeployable", deployeds[2])
...
assertEquals(3, len(previewBlock.blocks[0].block.blocks))
assertTrue("app-dependencies-ShopDatabase" in previewBlock.blocks[0].block.blocks[0].description)
assertTrue(previewBlock.blocks[0].block.blocks[1].parallel)
assertTrue("app-dependencies-ShopBackend" in previewBlock.blocks[0].block.blocks[1].description)
assertTrue("app-dependencies-ShopServices" in previewBlock.blocks[0].block.blocks[1].description)
assertTrue("app-dependencies-ShopFrontend" in previewBlock.blocks[0].block.blocks[2].description)
